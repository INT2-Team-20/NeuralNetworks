...
class NeuralN(nn.Module):
    ...
    def forward(self, x):
        #print(inputs.shape)
        x = self.LReLU(self.BNorm1(self.Conv1(x))) #110
       # print(x.shape)
       # print("initil "+ str(x.size()))
        x = self.Pool(x)
        x = self.Dropout(x)
         #55
...
neuNet.load_state_dict(torch.load('./Flowers8-380-603.pth'))

criterion = nn.CrossEntropyLoss()
optimizer = torch.optim.Adam(neuNet.parameters(),  0.000001, weight_decay=0.01) #previous rate:0.00005  , 0.001
bestAccuracy = 0
bestTAccuracy = 0
